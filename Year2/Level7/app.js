...
// Year 2 Level 7 — "Everything Bagel"
// 7 keys (C/G/D/F maj + Am/Em/Gm), 4/4 + 3/4 + 6/8, Easy/Medium/Harder
...
(function () {
  ...
  const KEY_DEFS = {
    "C":  { tonic: 60, mode: "major", abcKey: "C",  usesFlats: false,
            allowedPitches: [52,53,55,57,59,60,62,64,65,67,69,71,72,74,76,77,79],
            startPitches: [60, 72], endPitches: [60, 72],
            vChordPCs: [7, 11, 2] },
    "G":  { tonic: 55, mode: "major", abcKey: "G",  usesFlats: false,
            allowedPitches: [52,54,55,57,59,60,62,64,66,67,69,71,72,74,76,78,79],
            startPitches: [55, 67], endPitches: [55, 67],
            vChordPCs: [2, 6, 9] },
    "D":  { tonic: 62, mode: "major", abcKey: "D",  usesFlats: false,
            allowedPitches: [52,54,55,57,59,61,62,64,66,67,69,71,73,74,76,78,79],
            startPitches: [62, 74], endPitches: [62, 74],
            vChordPCs: [9, 1, 4] },
    "F":  { tonic: 53, mode: "major", abcKey: "F",  usesFlats: true,
            allowedPitches: [52,53,55,57,58,60,62,64,65,67,69,70,72,74,76,77,79],
            startPitches: [53, 65, 77], endPitches: [53, 65, 77],
            vChordPCs: [0, 4, 7] },
    "Am": { tonic: 57, mode: "minor", abcKey: "Am", usesFlats: false,
            allowedPitches: [52,53,55,56,57,59,60,62,64,65,67,68,69,71,72,74,76,77,79,80],
            startPitches: [57, 69], endPitches: [57, 69],
            vChordPCs: [4, 8, 11] },
    "Em": { tonic: 52, mode: "minor", abcKey: "Em", usesFlats: false,
            allowedPitches: [52,54,55,57,59,60,62,63,64,66,67,69,71,72,74,75,76,78,79],
            startPitches: [52, 64, 76], endPitches: [52, 64, 76],
            vChordPCs: [11, 3, 6] },
    "Gm": { tonic: 55, mode: "minor", abcKey: "Gm", usesFlats: true,
            allowedPitches: [53,54,55,57,58,60,62,63,65,66,67,69,70,72,74,75,77,78,79],
            startPitches: [55, 67], endPitches: [55, 67],
            vChordPCs: [2, 6, 9] },
  };
  ...
  function getSecPerQuarterBeat(bpm, meter) {
    if (meter === "6/8") return 40 / bpm;
    return 60 / bpm;
  }

  function getQuarterBeatsPerMeasure(meter) {
    if (meter === "4/4") return 4;
    return 3;
  }

  function isTriadPitches(p1, p2, p3) {
    const pcs = [p1 % 12, p2 % 12, p3 % 12];
    const intervals = new Set();
    for (let i = 0; i < 3; i++) {
      for (let j = i + 1; j < 3; j++) {
        const diff = Math.abs(pcs[i] - pcs[j]);
        intervals.add(Math.min(diff, 12 - diff));
      }
    }
    const arr = [...intervals].sort((a, b) => a - b);
    if (arr.length !== 3) return false;
    if (arr[0] === 3 && arr[1] === 4 && arr[2] === 5) return true;
    if (arr[0] === 3 && arr[1] === 3 && arr[2] === 6) return true;
    if (arr[0] === 4 && arr[1] === 4 && arr[2] === 4) return true;
    return false;
  }
  ...
  function pickNextPitch(prevPitch, prevInterval, scalePitches, keyDef, context) {
    ...
    for (let i = 0; i < scalePitches.length; i++) {
      ...
      if (stepsAway === 0) continue;

      // Rule: no skips larger than a 5th (4 scale steps)
      // Exception: octave allowed when current duration >= 2 (half note or longer)
      if (stepsAway > maxScaleSteps) {
        if (semitonesAway === 12 && context.currentDuration >= 2) {
          // Allow octave in slow passages — reduced weight
        } else {
          continue;
        }
      }
      ...
      else if (stepsAway === 4) weight *= 0.3;
      else weight *= 0.08;

      // Skip compensation: after a skip, prefer step in opposite direction
      if (prevInterval !== 0) {
        ...
        if (prevWasSkip) {
          if (isStep && curDir !== prevDir) weight *= 4.0;
            ...
            // Consecutive skips in same direction — only if outlining a triad
            if (context.prevPrevPitch != null &&
                isTriadPitches(context.prevPrevPitch, prevPitch, pitch)) {
              weight *= 0.8;
            } else {
              weight *= 0.03;
            }
      ...
      // Pitch repetition avoidance
      if (pitch === prevPitch) weight *= 0.03;

      // V chord emphasis in measures 4 and 7
      if (context.isVChord) {
        const pc = pitch % 12;
      ...
      // Leading tone logic (minor keys)
      if (isRaised7th(pitch, keyDef)) {
        weight *= (pitch > prevPitch) ? 1.5 : 0.05;
      }
      if (isRaised7th(prevPitch, keyDef)) {
        const tonicPC = keyDef.tonic % 12;
        if (pitch % 12 === tonicPC && pitch === prevPitch + 1) {
          weight *= 100;
        } else {
          continue;
        }
      ...
      if (weight > 0) candidates.push({ pitch, weight });
    }
  ...
  const RHYTHM_POOLS = {
    "4/4": {
      easy: [
        { pattern: [4],            weight: 5  },
        { pattern: [3, 1],         weight: 15 },
        { pattern: [2, 2],         weight: 20 },
        { pattern: [2, 1, 1],      weight: 20 },
        { pattern: [1, 1, 2],      weight: 15 },
        { pattern: [1, 1, 1, 1],   weight: 15 },
        { pattern: [1, 3],         weight: 10 },
      ],
      medium: [
        { pattern: [2, 2],               weight: 8  },
        { pattern: [2, 1, 1],            weight: 10 },
        { pattern: [1, 1, 2],            weight: 8  },
        { pattern: [1, 1, 1, 1],         weight: 12 },
        { pattern: [2, 0.5, 0.5, 1],     weight: 10 },
        { pattern: [1, 0.5, 0.5, 2],     weight: 10 },
        { pattern: [1, 1, 0.5, 0.5, 1],  weight: 10 },
        { pattern: [0.5, 0.5, 1, 1, 1],  weight: 10 },
        { pattern: [1.5, 0.5, 1, 1],     weight: 8  },
        { pattern: [1, 1, 1.5, 0.5],     weight: 8  },
        { pattern: [3, 1],               weight: 6  },
      ],
      harder: [
        { pattern: [1, 1, 1, 1],                  weight: 8  },
        { pattern: [2, 0.5, 0.5, 1],              weight: 8  },
        { pattern: [0.5, 0.5, 0.5, 0.5, 1, 1],    weight: 10 },
        { pattern: [1, 0.5, 0.5, 0.5, 0.5, 1],    weight: 10 },
        { pattern: [1, 1, 0.5, 0.5, 0.5, 0.5],    weight: 10 },
        { pattern: [0.5, 0.5, 1, 0.5, 0.5, 1],    weight: 8  },
        { pattern: [1.5, 0.5, 1.5, 0.5],          weight: 8  },
        { pattern: [0.75, 0.25, 0.5, 0.5, 1, 1],  weight: 6  },
        { pattern: [1, 1, 0.75, 0.25, 0.5, 0.5],  weight: 6  },
        { pattern: [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 1], weight: 6 },
        { pattern: [1, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], weight: 6 },
        { pattern: [2, 1, 0.5, 0.5],              weight: 6  },
        { pattern: [0.5, 0.5, 1, 2],              weight: 4  },
      ],
    },
    "3/4": {
      easy: [
        { pattern: [3],         weight: 8  },
        { pattern: [2, 1],      weight: 20 },
        { pattern: [1, 2],      weight: 15 },
        { pattern: [1, 1, 1],   weight: 20 },
      ],
      medium: [
        { pattern: [1, 1, 1],            weight: 12 },
        { pattern: [2, 1],               weight: 8  },
        { pattern: [1, 2],               weight: 8  },
        { pattern: [1, 0.5, 0.5, 1],     weight: 12 },
        { pattern: [0.5, 0.5, 1, 1],     weight: 10 },
        { pattern: [1, 1, 0.5, 0.5],     weight: 10 },
        { pattern: [1.5, 0.5, 1],        weight: 8  },
        { pattern: [1, 1.5, 0.5],        weight: 8  },
        { pattern: [0.5, 0.5, 0.5, 0.5, 1], weight: 6 },
      ],
      harder: [
        { pattern: [1, 1, 1],                  weight: 6  },
        { pattern: [1, 0.5, 0.5, 1],           weight: 8  },
        { pattern: [0.5, 0.5, 0.5, 0.5, 1],    weight: 10 },
        { pattern: [1, 0.5, 0.5, 0.5, 0.5],    weight: 10 },
        { pattern: [0.5, 0.5, 1, 0.5, 0.5],    weight: 8  },
        { pattern: [0.75, 0.25, 0.5, 0.5, 1],  weight: 6  },
        { pattern: [1, 0.75, 0.25, 0.5, 0.5],  weight: 6  },
        { pattern: [0.5, 0.5, 0.5, 0.5, 0.5, 0.5], weight: 8 },
        { pattern: [1.5, 0.5, 1],              weight: 6  },
        { pattern: [2, 0.5, 0.5],              weight: 6  },
        { pattern: [0.5, 0.5, 2],              weight: 4  },
      ],
    },
    "6/8": {
      medium: [
        { pattern: [3],                          weight: 8  },
        { pattern: [1.5, 1.5],                   weight: 15 },
        { pattern: [0.5, 0.5, 0.5, 1.5],         weight: 12 },
        { pattern: [1.5, 0.5, 0.5, 0.5],         weight: 12 },
      ...
      harder: [
        { pattern: [1.5, 1.5],                   weight: 8  },
        { pattern: [0.5, 0.5, 0.5, 1.5],         weight: 10 },
        { pattern: [1.5, 0.5, 0.5, 0.5],         weight: 10 },
        { pattern: [0.5, 0.5, 0.5, 0.5, 0.5, 0.5], weight: 10 },
        { pattern: [1, 0.5, 1, 0.5],             weight: 10 },
        { pattern: [1, 0.5, 0.5, 0.5, 0.5],       weight: 8  },
        { pattern: [0.5, 0.5, 0.5, 1, 0.5],       weight: 8  },
        { pattern: [0.5, 1, 1.5],                weight: 6  },
        { pattern: [1, 0.5, 0.5, 1],             weight: 8  },
        { pattern: [0.5, 0.5, 1, 1],             weight: 6  },
      ],
    },
  };

  function pickRhythmPattern(meter, difficulty) {
    const pool = RHYTHM_POOLS[meter][difficulty];
  ...
  // ==========================================================
  // 5. MELODY GENERATION (8 measures)
  // ==========================================================
  ...
  function pickCadencePitch(currentPitch, scalePitches, keyDef) {
    ...
    if (keyDef.mode === "minor") {
      const raised7ths = scalePitches.filter(p => isRaised7th(p, keyDef) && Math.abs(p - closestEnd) <= 3);
      if (raised7ths.length > 0) {
        return raised7ths.reduce((a, b) =>
          Math.abs(b - currentPitch) < Math.abs(a - currentPitch) ? b : a
        );
      }
    }
  ...
  function generateMelody(keyName, meter, difficulty, numMeasures) {
    ...
    const qbPerMeasure = getQuarterBeatsPerMeasure(meter);

    let currentPitch = keyDef.startPitches[
      Math.floor(seededRandom() * keyDef.startPitches.length)
    ];
    let prevInterval = 0;
    let prevPrevPitch = null;
    const measures = [];

    const restChance = difficulty === "easy" ? 0 : (difficulty === "medium" ? 0.06 : 0.08);
    ...
    for (let m = 0; m < numMeasures; m++) {
      ...
      // Last measure: tonic whole/dotted note
      if (isLast) {
        const endPitch = keyDef.endPitches.reduce((a, b) =>
          Math.abs(b - currentPitch) < Math.abs(a - currentPitch) ? b : a
        );
        measures.push([{ pitch: endPitch, duration: qbPerMeasure, isRest: false }]);
        continue;
      }

      const rhythm = pickRhythmPattern(meter, difficulty);

      // Sequence in measure 6 (0-indexed 5) based on measure 5 (0-indexed 4)
      if (m === 5 && measures.length >= 5 && seededRandom() < 0.6) {
        ...
        prevPrevPitch = seqNotes.length >= 2 ? seqNotes[seqNotes.length - 2].pitch : currentPitch;
  ...
  function buildExpectedNotes(measures, bpm, meter) {
    const secPerQuarterBeat = getSecPerQuarterBeat(bpm, meter);
    const notes = [];
    let time = 0;
  ...
  function render(abcString) {
    ...
    const tuneArr = ABCJS.renderAbc(el, abcString, {
      ...
      staffwidth: 900,
      paddingtop: 10,
      paddingbottom: (window.innerWidth <= 600 ? 5 : window.innerWidth <= 1024 ? 10 : 20),
      paddingleft: 20,
      paddingright: 20,
      scale: 1.3,
      add_classes: true,
  ...
  function scheduleMetronome(audioCtx, bpm, meter, numMeasures, countInBars, dest) {
    ...
    if (meter === "6/8") {
      ...
      for (let e = 0; e < totalEighths; e++) {
        ...
        playClick(audioCtx, time, (e % 3) === 0, dest);
      }
      ...
      return {
        ...
        startTime,
        countInClicks: countInBars * eighthsPerMeasure,
        clickInterval: secPerEighth,
      };
    } else {
      ...
      for (let b = 0; b < totalBeats; b++) {
        ...
        const isAccent = beatInMeasure === 0 || (meter === "4/4" && beatInMeasure === 2);
      ...
      return {
        ...
        startTime,
        countInClicks: countInBars * beatsPerMeasure,
        clickInterval: secPerBeat,
      };
    }
  }
  ...
  function scoreMelody(expected, detected, recordingDuration) {
    ...
    // Pass 2: retry unmatched with tighter window
  ...
  async function captureResultImage() {
    ...
    if (isDaily) {
      const dailyDate = attempt.settings.dailyDate;
  ...
  function onDifficultyChange() {
    ...
    const sixEighthOption = meterSelect.querySelector('option[value="6/8"]');

    if (diff === "easy") {
      if (sixEighthOption) sixEighthOption.disabled = true;
      if (meterSelect.value === "6/8") {
        meterSelect.value = "4/4";
      }
    } else {
      if (sixEighthOption) sixEighthOption.disabled = false;
    }
  ...
  async function startRecording() {
    ...
    const metroInfo = scheduleMetronome(audioCtx, currentBpm, currentMeter, currentNumMeasures, countInBars, recordMetronomeGain);

    const countInClicks = metroInfo.countInClicks;
    const clickIntervalMs = metroInfo.clickInterval * 1000;
    ...
    const countInMs = countInClicks * clickIntervalMs;
    ...
    const secPerQuarterBeat = getSecPerQuarterBeat(currentBpm, currentMeter);
    const qbPerMeasure = getQuarterBeatsPerMeasure(currentMeter);
    const melodyDurationMs = currentNumMeasures * qbPerMeasure * secPerQuarterBeat * 1000;
  ...
  // ==========================================================
  // 16b. MELODY PLAYBACK (meter-aware)
  // ==========================================================
  ...
  const MIDI_SAMPLE_MAP = {
    ...
    61: "audio/Cs-Db1.mp3",
    62: "audio/D1.wav",
    63: "audio/Ds1-Eb1.wav",
    64: "audio/E1.wav",
    65: "audio/F1.wav",
    66: "audio/Fs1-Gb1.mp3",
    67: "audio/G1.wav",
    68: "audio/Gs1-Ab1.mp3",
    69: "audio/A1.wav",
    70: "audio/As1-Bb1.mp3",
    71: "audio/B1.mp3",
    72: "audio/C2.wav",
    73: "audio/Cs2-Db2.wav",
    74: "audio/D2.wav",
    75: "audio/Ds2-Eb2.wav",
    76: "audio/E2.wav",
    77: "audio/F2.mp3",
    78: "audio/Fs2-Gb2.wav",
    79: "audio/G2.wav",
    80: "audio/Gs2-Ab2.mp3",
  };
  ...
  async function loadSampleBuffers(ctx) {
    ...
    await Promise.all(entries.map(async function ([midi, url]) {
      ...
      try {
        ...
        // Sample missing — will skip playback for this note
      }
    }));
    samplesLoaded = true;
  }
  ...
  async function startPlayback() {
    ...
    const countInBars = 1;
    const metroInfo = scheduleMetronome(playbackCtx, currentBpm, currentMeter, currentNumMeasures, countInBars, playbackMasterGain);
    ...
    const melodyBaseTime = metroInfo.countInEndTime;
    const allEls = getAllNoteRestEls();
  ...
  const SCORE_HISTORY_KEY = "sightreading-score-history-year2-level7";
  ...
  function dailyChallenge() {
    ...
    const availableMeters = difficulty === "easy" ? ["4/4", "3/4"] : ["4/4", "3/4", "6/8"];
    const meter = availableMeters[Math.floor(seededRandom() * availableMeters.length)];
    const numMeasures = 8;
    ...
    // Update 6/8 availability in UI
    onDifficultyChange.__skipGenerate = true;
    var evt = new Event("change");
    document.getElementById("difficultySelect").dispatchEvent(evt);
    onDifficultyChange.__skipGenerate = false;
  }
