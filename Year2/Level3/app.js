...
// Year 2 Level 3 — C Major & A Minor, arpeggios in 3/8
...
(function () {
  ...
  // Year 2 Level 3 — C major & A minor, arpeggios on strings 4-3-2, in 3/8
  const KEY_DEFS = {
  ...
  // Harmonic floor plans — each chord spans 2 bars of 3/8 (equivalent to 1 bar of 2/4)
  const FLOOR_PLANS = {
    "C": {
      8:  [["C"],       ["C"],       ["F", "Dm"], ["F", "Dm"],
           ["G", "G7"], ["G", "G7"], ["C"],       ["C"]],
  ...
  // ==========================================================
  // 3. MEASURE PATTERN ENGINE (3/8)
  // ==========================================================

  // Each pattern fills one 3/8 measure (1.5 quarter-note beats = 6 sixteenths).
  // Durations are in quarter-note units. Strings: 4=p (thumb), 3=i (index), 2=m (middle).
  const MEASURE_PATTERNS = [
    { id: "EEE",    durs: [0.5, 0.5, 0.5],                                    strings: [4, 3, 2],          weight: 20 },
    { id: "ESSSS",  durs: [0.5, 0.25, 0.25, 0.25, 0.25],                      strings: [4, 3, 2, 3, 2],    weight: 25 },
    { id: "SSSSSS", durs: [0.25, 0.25, 0.25, 0.25, 0.25, 0.25],               strings: [4, 3, 2, 3, 2, 3], weight: 15 },
    { id: "ESSE",   durs: [0.5, 0.25, 0.25, 0.5],                             strings: [4, 3, 2, 3],       weight: 20 },
    { id: "SSSSE",  durs: [0.25, 0.25, 0.25, 0.25, 0.5],                      strings: [4, 3, 2, 3, 2],    weight: 15 },
    { id: "DQ",     durs: [1.5],                                               strings: [4],                weight: 5  },
  ];

  function pickMeasurePattern() {
    const total = MEASURE_PATTERNS.reduce((s, p) => s + p.weight, 0);
    let r = seededRandom() * total;
    for (const p of MEASURE_PATTERNS) {
      r -= p.weight;
      if (r <= 0) return p;
    }
    return MEASURE_PATTERNS[MEASURE_PATTERNS.length - 1];
  }

  function assignNotesToMeasure(pattern, chord) {
    const notes = [];
  ...
  function generateMelody(keyName, meter, difficulty, numMeasures) {
    ...
    for (let m = 0; m < numMeasures; m++) {
      ...
      if (isLast) {
        ...
        let finalNotes;
        switch (pattern) {
          case 0: // EEE ending on str2
            finalNotes = [
              { pitch: str4, duration: 0.5, isRest: false },
              { pitch: str3, duration: 0.5, isRest: false },
              { pitch: str2, duration: 0.5, isRest: false },
            ];
            break;
          case 1: // EEE ending on tonic
            finalNotes = [
              { pitch: str4, duration: 0.5, isRest: false },
              { pitch: str3, duration: 0.5, isRest: false },
              { pitch: tonicPitch, duration: 0.5, isRest: false },
            ];
            break;
          case 2: // Dotted quarter on tonic
            finalNotes = [{ pitch: tonicPitch, duration: 1.5, isRest: false }];
            break;
          case 3: // ESS approach + E on tonic (tied when str2 === tonic)
            finalNotes = [
              { pitch: str4, duration: 0.5, isRest: false },
            ...
            break;
          case 4: // Dotted quarter with chord annotation
            finalNotes = [{ pitch: tonicPitch, duration: 1.5, isRest: false, annotation: keyName }];
            break;
          case 5: // Dyad: str4 + tonic as dotted quarter
            finalNotes = [{ pitch: tonicPitch, duration: 1.5, isRest: false, chordPitch: str4 }];
            break;
          case 6: // E on bass + quarter on tonic
            finalNotes = [
              { pitch: str4, duration: 0.5, isRest: false },
      ...
      // Pick one measure pattern for this 3/8 bar
      const pat = pickMeasurePattern();
      const notes = assignNotesToMeasure(pat, chord);
      measures.push(notes);
    }
  ...
  function melodyToAbc(measures, keyDef, meter) {
    let abc = "X:1\nM:" + meter + "\nL:1/16\n%%stretchlast true\nK:" + keyDef.abcKey + "\n";
    // In 3/8, beam all notes within the measure together (6 sixteenths per measure)
    const sixteenthsPerBeat = 6;
    ...
    for (let i = 0; i < measures.length; i++) {
      ...
      for (let j = 0; j < measure.length; j++) {
        ...
        // Beaming: beam all sub-beat notes within the measure together
  ...
  // Build a flat list of expected notes with timing info
  // BPM = quarter-note BPM; durations are in quarter-note units
      ...
      for (const note of measure) {
        if (note.isRest) {
          ...
          notes.push({
            ...
            name: "rest",
            startTime: time,
            duration: note.duration * secPerQuarter,
          ...
          if (prevTied && prev && !prev.isRest && prev.midi === note.pitch) {
            prev.duration += note.duration * secPerQuarter;
        ...
        time += note.duration * secPerQuarter;
      }
    }
    return notes;
  }
  ...
  // ==========================================================
  // 9. METRONOME (3/8 — clicks on eighth-note beats)
  // ==========================================================
  ...
  function scheduleMetronome(audioCtx, bpm, meter, numMeasures, countInBars, dest) {
    const secPerQuarter = 60 / bpm;
    const secPerEighth = secPerQuarter / 2;
    const eighthsPerMeasure = 3;
    ...
    for (let e = 0; e < totalEighths; e++) {
      ...
      const isAccent = (e % eighthsPerMeasure) === 0;
  ...
  function scoreMelody(expected, detected, recordingDuration) {
    ...
    // Second pass: retry unmatched notes
  ...
  async function startPlayback() {
    ...
    // Count-in (1 bar of 3/8 = 3 eighth-note clicks)
    const countInBars = 1;
    const countInEighths = countInBars * eighthsPerMeasure;
    ...
    for (let e = 0; e < countInEighths; e++) {
      ...
      playClick(playbackCtx, time, e === 0, playbackMasterGain);
    }
    ...
    // Schedule melody metronome clicks (on each eighth note)
    const totalEighths = currentNumMeasures * eighthsPerMeasure;
    for (let e = 0; e < totalEighths; e++) {
      const time = melodyBaseTime + e * secPerEighth;
      playClick(playbackCtx, time, (e % eighthsPerMeasure) === 0, playbackMasterGain);
    }
  ...
  const SCORE_HISTORY_KEY = "sightreading-score-history-year2-level3";
